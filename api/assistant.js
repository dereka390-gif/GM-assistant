...
async function callOpenRouter(systemPrompt,userPrompt){
  ...
  return {answer,provider:'OpenRouter Free',model:data?.model||'openrouter/free'};
}
...
async function callOpenAI(systemPrompt,userPrompt){
  ...
  return {answer,provider:'OpenAI',model:payload.model};
}

function localFallback(mode,question,context=[]){
  if(mode==='osm'){
    const excerpts=context.slice(0,5).map(x=>{
      const text=String(x.content||'').replace(/\s+/g,' ').trim();
      return `OSM page ${x.page_number}: ${text.slice(0,700)}${text.length>700?'…':''}`;
    });
    return {
      answer:`I found the most relevant OSM sections for “${question}”.\n\n${excerpts.join('\n\n')}\n\nThese are direct OSM excerpts rather than a generated interpretation. Use the cited page numbers to verify the exact standard.`,
      provider:'GM Assistant Local',model:'OSM retrieval fallback'
    };
  }

  if(mode==='communication'){
    return {
      answer:`TEAM UPDATE\n\n${question}\n\nFOCUS TODAY\n• Keep the shift organized and communicate early.\n• Protect speed without sacrificing accuracy or hospitality.\n• Make it right for every guest.\n• Recognize wins and coach opportunities in the moment.\n\nLet’s finish strong and take care of each other and our guests.`,
      provider:'GM Assistant Local',model:'Communication fallback'
    };
  }
  ...
  actions.push('Recognize what is already working so the team knows what to repeat.');
  actions.push('Track the result daily and change only one or two variables at a time so you know what actually improved performance.');

  return {
    answer:`GM Coach fallback\n\nBased on your request — “${question}” — focus on the largest controllable opportunity first.\n\n${actions.slice(0,5).map((a,i)=>`${i+1}. ${a}`).join('\n')}\n\nThe live language model is temporarily unavailable, so this response is generated by GM Assistant’s built-in operations logic.`,
    provider:'GM Assistant Local',model:'Operations fallback'
  };
}
...
export default async function handler(req,res){
  ...
  if(process.env.OPENROUTER_API_KEY){
    try{result=await callOpenRouter(instructions,input);}catch(e){providerErrors.push(`OpenRouter: ${e.message}`);}
  }
  if(!result&&process.env.OPENAI_API_KEY){
    try{result=await callOpenAI(instructions,input);}catch(e){providerErrors.push(`OpenAI: ${e.message}`);}
  }
  ...
  return res.status(200).json({
    ...
    user:user.id,
    mode,
    degraded:result.provider==='GM Assistant Local',
    providerErrors,
